...
setInterval(function () {
  async.each(Object.keys(users), function (key, callback) {
    var socket = realtime.io.sockets.sockets.get(key)
...
function extractNoteIdFromSocket (socket) {
  ...
  var hostUrl = new URL(referer)
  var noteId =
    config.urlpath
      ? hostUrl.pathname.slice(config.urlpath.length + 1, hostUrl.pathname.length).split('/')[1]
      : hostUrl.pathname.split('/')[1]
...
function updateHistory (userId, note, time) {
  var noteId = note.alias ? note.alias : LZString.compressToEncodedURIComponent(note.id)
