...
module.exports = function (sequelize, DataTypes) {
  ...
  User.prototype.verifyPassword = async function (attempt) {
    return Scrypt.verif(Buffer.from(this.password, 'hex'), attempt)
  }
  ...
  var hashPasswordHook = async function (user) {
    if (!user.changed('password')) { return }

    var hash = await Scrypt.kdf(user.password, await Scrypt.pickParams(0.1)).toString('hex')
