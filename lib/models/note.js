...
module.exports = function (sequelize, DataTypes) {
  var Note = sequelize.define('Note', {
    ...
    hooks: {
      ...
      afterCreate: function (note, options) {
  ...
  Note.parseNoteId = function (noteId, callback) {
    async.series({
      ...
      parseNoteIdByLZString: function (_callback) {
        ...
        try {
          var id = LZString.decompressFromEncodedURIComponent(noteId)
  ...
  Note.parseMeta = function (meta) {
    ...
    if (meta) {
      ...
      if (meta.disqus && typeof meta.disqus === 'string' && meta.disqus.match(/^[a-z0-9]$/i)) { _meta.disqus = meta.disqus }
